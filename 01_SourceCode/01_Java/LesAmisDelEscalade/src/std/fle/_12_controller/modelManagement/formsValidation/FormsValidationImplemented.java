package std.fle._12_controller.modelManagement.formsValidation;
...
@Service
public class FormsValidationImplemented implements FormsValidation {
	...
	public void checkStateNotEmpty(ClimbingSiteSFC climbingSiteSFC, String modelAttributeName, BindingResult result) {
		try {
			if (FredParser.toInteger(climbingSiteSFC.getClimbingSiteStateId()) == 19)
	...
	public void checkStateNotEmpty(ClimbingTopoSFC climbingTopoSFC, String modelAttributeName, BindingResult result) {
		try {
			if (FredParser.toInteger(climbingTopoSFC.getState()) == 19)
				throw new NullPointerException();
		} catch (Exception e) {
			SpringValidationError.addError(stateEmpty, modelAttributeName, "state",
					stateEmptyDefaultMessage, result);
		}
	}
	...
	public void checkCountyNotEmpty(ClimbingSiteSFC climbingSiteSFC, String modelAttributeName, BindingResult result) {
		try {
			if (FredParser.toInteger(climbingSiteSFC.getClimbingSiteCountyId()) == 102)
				throw new NullPointerException();
		} catch (Exception e) {
